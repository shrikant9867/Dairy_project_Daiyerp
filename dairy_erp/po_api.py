...
def get_po_list():
	...
	try:
		po_list = frappe.db.sql("""select status,name,schedule_date,supplier,tc_name,terms,status from `tabPurchase Order` where company = '{0}' and status in ('To Receive and Bill') order by creation desc limit 10""".format(get_seesion_company_datails().get('company')),as_dict=1)
		for row in po_list:
