...
def feed_fooder_advance(doc, method):
	...
	if ('Vlcc Manager' in roles or 'Vlcc Operator' in roles):
		if doc.local_sale and doc.local_sale_type == "Feed And Fodder Advance" and doc.farmer and doc.no_of_instalment:
			...
			farmer_advance.feed_and_fodder_si = doc.name# '<a href="#Form/Sales Invoice/'+doc.name+'">'+doc.name+'</a>'
...
def get_item_by_customer_type(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters.get("items_dict"):
		p_item = [item for item in filters.get("items_dict")]
