...
def local_sale_list():
	response_dict = {}
	try:
		la_list = frappe.db.sql("""select name,customer_or_farmer,posting_date,additional_discount_percentage,grand_total,status from `tabSales Invoice` where local_sale =1 and company = '{0}'order by creation desc limit 10 """.format(get_seesion_company_datails().get('company')),as_dict=1)
		for row in la_list:
