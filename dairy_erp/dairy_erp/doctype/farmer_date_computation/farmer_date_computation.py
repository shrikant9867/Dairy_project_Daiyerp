...
class FarmerDateComputation(Document):
	pass

def date_permission_query(user):
	...
	if date_list:
		if user != 'Administrator' and 'Vlcc Manager' in roles:
			return """`tabFarmer Date Computation`.name in ({date})""".format(date=','.join(date_list))
	else:
		if user != 'Administrator':
			return """`tabFarmer Date Computation`.name = 'Guest' """
