...
def get_receivable_data(filters):
	...
	filters["customer"] = frappe.db.get_value("Village Level Collection Centre",filters.get("camp"),"name1")
...
def get_payable_data(filters):
	...
	filters["supplier"] = frappe.db.get_value("User",filters.get("vlcc"),"first_name")
...
def get_vlccs():
	return [ [f.get('name'), f.get('name1')] for f in frappe.get_all("Village Level Collection Centre", fields=["name", "name1"]) ]
