...
def create_si():
	...
	for row in docs:
		...
		if len(cur_cycl):
			if cur_cycl[0].get('name') in req_cycle_computation(row) and cur_cycl[0].get('name') not in cc and and not frappe.db.get_value("Farmer Settings",{'vlcc': row.get('vlcc')}, 'is_fpcr'):
