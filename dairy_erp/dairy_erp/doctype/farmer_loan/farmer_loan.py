...
class FarmerLoan(Document):
	def on_submit(self):
		not_req_cycl = frappe.db.sql("""
			...
			format(self.date_of_disbursement,self.emi_deduction_start_cycle),as_dict=1,debug=1)
		...
		req_cycle = frappe.db.sql("""
			...
			""".format(date=self.date_of_disbursement, cycle = ','.join(not_req_cycl_list),
				instalment = self.no_of_instalments),as_dict=1,debug=1)
